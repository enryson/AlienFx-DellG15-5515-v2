...
public:
    ...
    bool sendPayload(const uint8_t *payload, int size);
...
bool USBDeviceHandler::sendPayload(const uint8_t *payload, int size)
{
    int transferred;
    int r = libusb_interrupt_transfer(handle, LIBUSB_ENDPOINT_OUT | 1, (unsigned char *)payload, size, &transferred, 0);
